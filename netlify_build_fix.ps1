# Script de build personalizado para Netlify (PowerShell)
# Resolve problemas de build e configura o ambiente

Write-Host "🔧 Iniciando build personalizado para Netlify..." -ForegroundColor Green

# Verificar se estamos no ambiente do Netlify
if ($env:NETLIFY) {
    Write-Host "✅ Ambiente Netlify detectado" -ForegroundColor Green
    Write-Host "📊 Informações do build:" -ForegroundColor Cyan
    Write-Host "   - Build ID: $env:NETLIFY_BUILD_ID" -ForegroundColor Gray
    Write-Host "   - Deploy URL: $env:NETLIFY_DEPLOY_URL" -ForegroundColor Gray
    Write-Host "   - Site ID: $env:NETLIFY_SITE_ID" -ForegroundColor Gray
} else {
    Write-Host "⚠️  Executando fora do ambiente Netlify" -ForegroundColor Yellow
}
...
function Create-File {
    ...
    Write-Host "📝 Criando $FilePath..." -ForegroundColor Cyan
    ...
    Write-Host "✅ $FilePath criado" -ForegroundColor Green
}
...
idna==3.6
"@
    Create-File "requirements.txt" $requirementsContent
} else {
...
if (-not (Test-Path "functions/api.py")) {
    $apiContent = @'
import json
import os
import sys
...
'@
    Create-File "functions/api.py" $apiContent
} else {
    Write-Host "✅ functions/api.py já existe" -ForegroundColor Green
}
...
[[headers]]
  ...
  [headers.values]
    ...
    Access-Control-Allow-Methods = "POST, OPTIONS"
"@
    Create-File "netlify.toml" $netlifyContent
} else {
...
'@
    Create-File "index.html" $indexContent
} else {
...
# 8. Criar arquivo runtime.txt
if (-not (Test-Path "runtime.txt")) {
    Create-File "runtime.txt" "python-3.9.18"
} else {
...
if (-not (Test-Path ".python-version")) {
    Create-File ".python-version" "3.9.18"
} else {
    Write-Host "✅ .python-version já existe" -ForegroundColor Green
}

Write-Host "✅ Build personalizado concluído!" -ForegroundColor Green
...
# Verificar se estamos no ambiente do Netlify
if ($env:NETLIFY) {
    Write-Host "🌐 Ambiente Netlify detectado - instalando dependências..." -ForegroundColor Green
    
    # Instalar dependências Python
    Write-Host "📦 Instalando dependências Python..." -ForegroundColor Cyan
    pip install -r requirements.txt --quiet
    
    Write-Host "🎉 Deploy pronto! O Netlify deve conseguir fazer o build agora." -ForegroundColor Green
} else {
    Write-Host "💻 Ambiente local - para testar localmente:" -ForegroundColor Cyan
    Write-Host "   1. Adicione o PDF na pasta PDFs/" -ForegroundColor Gray
    Write-Host "   2. Execute: python -m flask run" -ForegroundColor Gray
    Write-Host "   3. Acesse: http://localhost:5000" -ForegroundColor Gray
}

Write-Host "✅ Script netlify_build_fix.ps1 executado com sucesso!" -ForegroundColor Green 